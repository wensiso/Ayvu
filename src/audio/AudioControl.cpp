...
AudioControl::AudioControl(QObject* parent) :
		QObject(parent), m_audioRunning(false), m_speakerOn(false), m_dtmfAudioManager(
				0) {
...
void AudioControl::toggleSpeaker() {
	m_speakerOn ? audioPCM.toggleSpeaker(false) : audioPCM.toggleSpeaker(true);
...
void AudioControl::toggleAudioOnOff() {
	...
	m_audioRunning ? audioPCM.stopPcmAudio() : audioPCM.startPcmAudio();
	m_audioRunning = !m_audioRunning;
	emit audioRunningSignal(m_audioRunning);
}

void AudioControl::toggleAudioOn() {
	if (!m_audioRunning) {
		audioPCM.startPcmAudio();
		m_audioRunning = !m_audioRunning;
		emit audioRunningSignal(m_audioRunning);
	}
}

void AudioControl::toggleAudioOff() {
	if (m_audioRunning) {
		audioPCM.stopPcmAudio();
		m_speakerOn = false;
		emit speakerOnSignal(m_speakerOn);
		m_audioRunning = !m_audioRunning;
		emit audioRunningSignal(m_audioRunning);
...
void AudioControl::lazyInitializDtmfPlayer() {
	if (m_dtmfAudioManager == 0) {
		int ret = audio_manager_get_handle(AUDIO_TYPE_VOICE_TONES, 0, true,
				&m_dtmfAudioManager);
		if (ret != 0) {
			qDebug() << "###AudioControl::audioManagerHandle() = " << ret;
		}
		ret = audio_manager_set_handle_routing_conditions(m_dtmfAudioManager,
